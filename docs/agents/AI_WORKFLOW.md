...
| Clean Docker install recovery | `docker compose -p fordewind-clean-install-test run --rm --no-deps app true`, then `docker compose -p fordewind-clean-install-test run --rm --no-deps --user "$(id -u):$(id -g)" app composer install --no-interaction` | Passed: the root preparation step created writable framework, bootstrap-cache, and Composer-cache directories; `package:discover` completed under the host UID. |
| --- | --- | --- |
